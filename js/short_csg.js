...
// # class CSG
// union, subtract, and intersect methods.
...
class CSG{
	...
	union(csg){
		let a=new Node(this.clone().polygons);let b=new Node(csg.clone().polygons);
	...
	inverse(){let csg=this.clone();csg.polygons.forEach(p=>p.flip());return csg;}
}

CSG.fromPolygons=function(polygons){let csg=new CSG();csg.polygons=polygons;return csg;}
...
class Vector {
	...
	lerp(a,t){return this.add(tv0.copy(a).sub(this).times(t));}
...
class Vertex{
	...
	interpolate(other,t){return new Vertex(this.pos.clone().lerp(other.pos,t),this.normal.clone().lerp(other.normal,t),this.uv&&other.uv&&this.uv.clone().lerp(other.uv,t),this.color&&other.color&&this.color.clone().lerp(other.color,t));}
}
...
class Plane{
	...
	switch (polygonType){
		case COPLANAR:(this.normal.dot(polygon.plane.normal) > 0 ? coplanarFront : coplanarBack).push(polygon);	break;
...
CSG.fromJSON=function(json){return CSG.fromPolygons(json.polygons.map(p=>new Polygon(p.vertices.map(v=> new Vertex(v.pos,v.normal,v.uv)),p.shared)));}
...
CSG.fromGeometry=function(geom,objectIndex){
	...
	if(geom.isGeometry){
		...
		for(let i=0;i<fs.length;i++){
			let f=fs[i];
			let vertices=[]
			for(let j=0;j<3;j++){vertices.push(new Vertex(vs[f[fm[j]]],f.vertexNormals[j],geom.faceVertexUvs[0][i][j]));polys.push(new Polygon(vertices,objectIndex));}
		}
	...
	for(let i=0,pli=0,l=index.length; i < l; i += 3,pli++){
		...
		for(let j=0;j<3;j++){
			let vi=index[i+j];
			let vp=vi*3;let vt=vi*2;
			let x=posattr.array[vp];let y=posattr.array[vp+1];let z=posattr.array[vp+2];	
			let nx=normalattr.array[vp];let ny=normalattr.array[vp+1];let nz=normalattr.array[vp+2];
			vertices[j]=new Vertex({x,y,z},{x:nx,y:ny,z:nz},uvattr&&{x:uvattr.array[vt],y:uvattr.array[vt+1],z:0},colorattr&&{x:colorattr.array[vt],y:colorattr.array[vt+1],z:colorattr.array[vt+2]});
		}
...
let nbuf3=(ct)=>{return{top:0,array:new Float32Array(ct),write:function(v){(this.array[this.top++]=v.x);(this.array[this.top++]=v.y);(this.array[this.top++]=v.z);}}}
let nbuf2=(ct)=>{return{top:0,array:new Float32Array(ct),write:function(v){(this.array[this.top++]=v.x);(this.array[this.top++]=v.y)}}}
	...
	if(!buffered){
		...
		geom.verticesNeedUpdate=geom.elementsNeedUpdate=geom.normalsNeedUpdate=true;
			...
			ps.forEach(p=>{
				...
				if(pvlen){if(pvs[0].color!==undefined){if(!colors)colors=nbuf3(triCount*3*3);};if(pvs[0].uv!==undefined){if(!uvs)uvs=nbuf2(triCount*2*3)};}
				for(let j=3;j<=pvlen;j++){
...
let spawnWorker=()=>{
	...
	worker.onmessage=function(e){let rslt=JSON.parse(e.data);let task=tasks[rslt.taskId];delete tasks[rslt.taskId];task.resolve(CSG.fromJSON(rslt.result));gWorker.busy=false;}
...
let getWorker=()=>{
	...
	if(gWorker && (!gWorker.busy)){gWorker.busy=true;return{then:(fn)=>{return fn(gWorker);}}}
	return{
